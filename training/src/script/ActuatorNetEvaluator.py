...
from ActuatorNet import ActuatorNet, HISTORY_SIZE, INPUT_SIZE, NUM_LAYERS, MAX_TORQUE, MAX_VELOCITY, MAX_ERROR, MAX_TEMPERATURE
import time
import os
...
class ActuatorNetEvaluator:
    ...
    def load_data(self, file_path):
        ...
        if 'timestamp_sec' in data.columns:
            self.time_values = data['timestamp_sec'].values
    ...
    def prepare_sequence_data(self, position_errors, velocities, temperatures, torques):
        ...
        temperatures = self.normalize_data(temperatures, -MAX_TEMPERATURE, MAX_TEMPERATURE) 
        ...
        for i in range(len(torques) - self.history_size + 1):
            X.append(np.column_stack((position_errors[i:i+self.history_size], 
                                      ...
                                      temperatures[i:i+self.history_size])))  
    ...
    def plot_data_visualization(self, y, predictions, position_errors, velocities, temperatures, 
        ...
        y_titles = ['Position Error [rad]', 'Velocity [units/s]', 'Temperature [Celsius]', 'Torque [Nm]', 'Model Error [Nm]']
    ...
    def plot_predictions(self, data_file, prediction_files, model_names, plot_vs_time=False, save_html=False, save_pdf=False, pdf_subplots=None, plot_config=None):
        ...
        plot_order = ['position_error', 'velocity', 'temperature', 'torque', 'prediction_error']
        ...
        # Acceleration plot
        add_trace_if_enabled('temperature',
            go.Scatter(x=x_axis, y=temperatures, mode='lines', name='Temperature',
                    line=dict(color='lightgreen', width=line_thickness)))
    ...
    def evaluate_model(self, X, y, position_errors, velocities, temperatures, torques, 
        ...
        self.plot_data_visualization(y, predictions, position_errors, velocities, temperatures, 
                                     rms_error, percentage_accuracy, total_inference_time, average_inference_time, 
